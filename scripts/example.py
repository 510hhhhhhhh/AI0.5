...
if env.isActionAvaliable(goal_x, goal_y, goal_yaw):  # 判断目标点是否可行
    controller.send_goal(env.navgoal)  # 发送目标
    print 'sending my_goal successful'
else:
    pass
...
while True:
    ...
    if (env.EnemyPose.enemy_dist != 0):# 没到跳出来
        ...
        print 'shoot======================================================='
        # time.sleep(0.5)
        if (env.EnemyPose.enemy_dist >1.5):  # 检测到敌人
            dist = env.EnemyPose.enemy_dist - 1
            yaw = (env.EnemyPose.enemy_yaw + env.Gimbal.enemy_yaw) * 3.1416 / 180
            ...
            if env.isActionAvaliable(goal_x, goal_y, goal_yaw):  # 判断目标点是否可行
                ...
                while(((goal_x-env.MyPose['x'])*(goal_x-env.MyPose['x']) +(goal_y-env.MyPose['y'])*(goal_y-env.MyPose['y'])) > 2.25 ):
                    distnow = (goal_x - env.MyPose['x']) * (goal_x - env.MyPose['x']) + (goal_y - env.MyPose['y']) * (goal_y - env.MyPose['y'])
        ...
        if (env.EnemyPose.enemy_dist < 1.5 and env.EnemyPose.enemy_dist > 0):
            print 'distance less than 1.5'
            print 'Enemy pose is yaw=%s\tdistance=%s\n' % (
                env.EnemyPose.enemy_yaw, env.EnemyPose.enemy_dist)
            goal_x = env.MyPose['x']
            goal_y = env.MyPose['y']
        ...
        controller.mode_switch(mode)  # 发送模式
        # plan.patrol_path(env,controller) #发送目标
        print 'no target for %s times!' % num
        if(num > 10):
            plan.random_path(env,controller)
            num = 0
            print '=========================patrol random goal!================================'
